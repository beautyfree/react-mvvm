...
describe('Common use of View and ChildView', () => {
  ...
  const testComponentObservability = (Component: FC, isObserver: boolean) => {
    ...
    Array(5)
      ...
      .forEach(() => {
        ...
        expect(
          screen.getByText((isObserver ? vm.n : n).toString())
        ).toBeInTheDocument()
      })
  }
  ...
  const propsAreEqual = (
    prevProps: EqualityCheckProps,
    nextProps: EqualityCheckProps
  ...
  const testPropsEqualityMechanism = (Component: FC<EqualityCheckProps>) => {
    ...
    expect(
      screen.getByRole('spinbutton', { name: 'Render count' })
    ).toHaveValue(1)
  ...
  describe('View', () => {
    ...
    test("Memo's propsAreEqual", () => {
      const View = view(SomeViewModel)<EqualityCheckProps>(
        ({ a, b }) => {
    ...
    test('ForwardRef', () => {
      const View = view(SomeViewModel)<unknown, HTMLDivElement>(
        forwardRef((_, ref) => <div ref={ref}>Component</div>)
      )
  ...
  describe('ChildView', () => {
    ...
    test("Memo's propsAreEqual", () => {
      const ChildView = childView<SomeViewModel>()<EqualityCheckProps>(
        ({ a, b }) => {
          const renderCount = useRef(0)
          renderCount.current++
    ...
    test('ForwardRef', () => {
      ...
      const View = view(SomeViewModel)<unknown, HTMLDivElement>(
        forwardRef((_, ref) => <ChildView ref={ref} />)
      )
  ...
  test('ChildViewComponent', () => {
    class SomeChildView extends ChildViewComponent<SomeViewModel> {
      render() {
        return (
          <div data-testid="is-equal">
            {JSON.stringify(this.viewModel === vm)}
          </div>
        )
      }
    }
    ...
    describe('Development', () => {
      ...
      test('Anonymous view model', () => {
        ...
        const AnonymousViewModel = (() =>
          class extends ViewModel {} as Constructable<ViewModel>)()
