import { childView, view, ViewModel } from 'react-mvvm'
...
export class MapViewModel extends ViewModel {
  ...
  @observable snakeCoordinates: TCoordinate[] = [3, 4, 5, 6].map((x) => ({
    y: 2,
    x,
  }))
  ...
  @action protected onViewMounted() {
    ...
    window.addEventListener(
      ...
      action((evt) => {
        if (
          ['ArrowUp', 'KeyW'].includes(evt.code) &&
          this.direction !== 'down'
        ) {
          this.nextDirection = 'up'
        } else if (
          ['ArrowRight', 'KeyD'].includes(evt.code) &&
          this.direction !== 'left'
        ) {
          this.nextDirection = 'right'
        } else if (
          ['ArrowDown', 'KeyS'].includes(evt.code) &&
          this.direction !== 'up'
        ) {
          this.nextDirection = 'down'
        } else if (
          ['ArrowLeft', 'KeyA'].includes(evt.code) &&
  ...
  private isCoordsEqual = (
    coordinate1: TCoordinate | null,
    coordinate2: TCoordinate | null
  ) =>
    !!(
      coordinate1 &&
      coordinate2 &&
