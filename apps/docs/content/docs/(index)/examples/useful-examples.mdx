...
### MobX React Library Configuration

You can choose between `mobx-react` and `mobx-react-lite` using a simple boolean flag. By default, the library uses `mobx-react-lite` (lighter and recommended for most cases). You can also access the configured library directly using the `getMobxReact` function.

```tsx
import { configure, getMobxReact, view, ViewModel } from 'react-mvvm'
import { observable, action, makeObservable } from 'mobx'

// Use mobx-react-lite (default, lighter)
configure({
  lite: true, // default
})

// Or use mobx-react (full-featured, includes Provider/Inject)
configure({
  lite: false,
})

// Access the configured library directly
const mobxReact = getMobxReact()
...
// The view will automatically use the configured library
